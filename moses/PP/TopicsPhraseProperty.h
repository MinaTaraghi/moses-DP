...
class TopicsPhraseProperty : public PhraseProperty
{
  friend std::ostream& operator<<(std::ostream &, const TopicsPhraseProperty &);

public:

  TopicsPhraseProperty() {};
  ...
  float GetJointCount() const {
    ...
    std::vector<float> GetTopics() const
    {
      return Topics;
    }
